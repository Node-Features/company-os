...
| `ADR-0006-daemon-boot-sequence.md` | Fix the Daemon's concrete boot sequence and tie it to the real `companyd` entrypoint | APPROVED | Read when changing `cmd/companyd/main.go` or the process startup/shutdown sequence. |
| `ADR-0007-concurrency-model.md` | State which operations are concurrent vs. serialized and why, with a compiled worked example | APPROVED | Read when adding concurrent dispatch, shared mutable state, or evaluating a synchronization choice. |
