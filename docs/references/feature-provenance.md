...
| Retry and recovery | Runtime | Temporal; Inngest as secondary reference | Pattern | Explicit retry policy, backoff, non-retryable failures, side-effect separation | Domain rules, idempotency, and policy gates constrain retries. |
| Scheduled and event-triggered work | Daemon | Inngest | Pattern | Cron/event triggers, flow control, steps that wake work | Daemon should wake authorized work instead of keeping expensive agents continuously active. |
